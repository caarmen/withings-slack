...
async def do(
    ...
    last_activity_data: ActivityData = await repo.get_latest_activity_by_user_and_type(
        fitbit_userid=fitbit_userid,
        type_id=new_activity_data.type_id,
    )

    await repo.create_activity_for_user(
        fitbit_userid=fitbit_userid,
        activity=new_activity_data,
    )
    all_time_top_activity_stats: TopActivityStats = (
        await repo.get_top_activity_stats_by_user_and_activity_type(
            fitbit_userid=fitbit_userid,
            type_id=new_activity_data.type_id,
        )
    )
